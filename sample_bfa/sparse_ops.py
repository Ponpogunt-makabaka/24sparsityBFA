import torch
from torch import autograd, nn
import torch.nn.functional as F

from itertools import repeat
# from torch._six import container_abcs


class Sparse(autograd.Function):
    """" Prune the unimprotant weight for the forwards phase but pass the gradient to dense weight using SR-STE in the backwards phase"""

    @staticmethod
    def forward(ctx, weight, N, M, decay = 0.0002):
        ctx.save_for_backward(weight)

        output = weight.clone()
        length = weight.numel()
        group = int(length/M)

        weight_temp = weight.detach().abs().reshape(group, M)
        index = torch.argsort(weight_temp, dim=1)[:, :int(M-N)]

        w_b = torch.ones(weight_temp.shape, device=weight_temp.device)
        w_b = w_b.scatter_(dim=1, index=index, value=0).reshape(weight.shape)
        ctx.mask = w_b
        ctx.decay = decay

        return output*w_b


    @staticmethod
    def backward(ctx, grad_output):

        weight, = ctx.saved_tensors
        return grad_output + ctx.decay * (1-ctx.mask) * weight, None, None
class Sparse_NHWC(autograd.Function):
    """" Prune the unimprotant edges for the forwards phase but pass the gradient to dense weight using SR-STE in the backwards phase"""

    @staticmethod
    def forward(ctx, weight, N, M, decay = 0.0002):

        ctx.save_for_backward(weight)
        output = weight.clone()
        out_ch, in_ch, kh, kw = weight.shape
        if in_ch % M != 0:
            w_b = torch.ones_like(weight)
            ctx.mask = w_b
            ctx.decay = decay
            return output * w_b

        weight_temp = weight.detach().abs().view(out_ch, in_ch // M, M, kh, kw)
        index = torch.argsort(weight_temp, dim=2)[:, :, :int(M - N), :, :]

        w_b = torch.ones_like(weight_temp)
        w_b = w_b.scatter_(dim=2, index=index, value=0).view_as(weight)

        ctx.mask = w_b
        ctx.decay = decay

        return output*w_b

    @staticmethod
    def backward(ctx, grad_output):

        weight, = ctx.saved_tensors
        return grad_output + ctx.decay * (1-ctx.mask) * weight, None, None

class SparseConv(nn.Conv2d):
    """" implement N:M sparse convolution layer """

    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros', N=2, M=4, **kwargs):
        self.N = N
        self.M = M
        super(SparseConv, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, padding_mode, **kwargs)

        # Mask freezing for Mode B (Static Masking)
        self.mask_frozen = False
        self.cached_mask = None

    def freeze_mask(self):
        """
        Freeze the sparsity mask.
        Calculates the mask once and caches it for reuse in forward pass.
        Used for Mode B (Static Masking) BFA attacks.
        """
        with torch.no_grad():
            self.cached_mask = self._calculate_mask()
        self.mask_frozen = True

    def unfreeze_mask(self):
        """
        Unfreeze the sparsity mask.
        The mask will be recalculated every forward pass.
        Used for Mode A (Dynamic Masking) BFA attacks.
        """
        self.mask_frozen = False
        self.cached_mask = None

    def _calculate_mask(self):
        """Calculate the N:M sparsity mask for current weights."""
        N, M = self.N, self.M
        weight = self.weight
        out_ch, in_ch, kh, kw = weight.shape
        if in_ch % M != 0:
            return torch.ones_like(weight)

        weight_temp = weight.abs().view(out_ch, in_ch // M, M, kh, kw)
        index = torch.argsort(weight_temp, dim=2)[:, :, :int(M - N), :, :]

        w_b = torch.ones_like(weight_temp)
        w_b = w_b.scatter_(dim=2, index=index, value=0).view_as(weight)

        return w_b.clone()

    def get_sparse_weights(self):

        return Sparse_NHWC.apply(self.weight, self.N, self.M)


    def forward(self, x):
        if self.mask_frozen and self.cached_mask is not None:
            # Mode B: Use frozen mask
            w = self.weight * self.cached_mask
        else:
            # Mode A: Dynamic mask calculation
            w = self.get_sparse_weights()

        x = F.conv2d(
            x, w, self.bias, self.stride, self.padding, self.dilation, self.groups
        )
        return x



class SparseLinear(nn.Linear):

    def __init__(self, in_features: int, out_features: int, bias: bool = True, N=2, M=4, decay = 0.0002, **kwargs):
        self.N = N
        self.M = M
        super(SparseLinear, self).__init__(in_features, out_features, bias = True)

        # Mask freezing for Mode B (Static Masking)
        self.mask_frozen = False
        self.cached_mask = None

    def freeze_mask(self):
        """
        Freeze the sparsity mask.
        Calculates the mask once and caches it for reuse in forward pass.
        Used for Mode B (Static Masking) BFA attacks.
        """
        with torch.no_grad():
            self.cached_mask = self._calculate_mask()
        self.mask_frozen = True

    def unfreeze_mask(self):
        """
        Unfreeze the sparsity mask.
        The mask will be recalculated every forward pass.
        Used for Mode A (Dynamic Masking) BFA attacks.
        """
        self.mask_frozen = False
        self.cached_mask = None

    def _calculate_mask(self):
        """Calculate the N:M sparsity mask for current weights."""
        N, M = self.N, self.M
        weight = self.weight
        length = weight.numel()
        group = int(length / M)

        weight_temp = weight.abs().reshape(group, M)
        index = torch.argsort(weight_temp, dim=1)[:, :int(M - N)]

        w_b = torch.ones(weight_temp.shape, device=weight_temp.device)
        w_b = w_b.scatter_(dim=1, index=index, value=0).reshape(weight.shape)

        return w_b.clone()

    def get_sparse_weights(self):

        return Sparse.apply(self.weight, self.N, self.M)


    def forward(self, x):
        if self.mask_frozen and self.cached_mask is not None:
            # Mode B: Use frozen mask
            w = self.weight * self.cached_mask
        else:
            # Mode A: Dynamic mask calculation
            w = self.get_sparse_weights()

        x = F.linear(x, w, self.bias)
        return x


    

    

