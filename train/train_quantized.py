#!/usr/bin/env python3
"""
Main training script for quantized ResNet-20 on CIFAR-10.
"""
...
from config import TRAINING, QUANTIZATION, DATASET, PATHS
...
def parse_args():
    ...
    # Model arguments
    parser.add_argument('--bit-width', type=int, default=QUANTIZATION['bit_width'],
...
def main():
    ...
    # Create model
    print(f'Creating ResNet-20 with {args.bit_width}-bit quantization...')
    base_model = resnet20(num_classes=DATASET['num_classes'])
    model = QuantizedResNet(base_model, bit_width=args.bit_width)
    ...
    # Save final model
    final_path = os.path.join(args.save_dir, 'trained_model.pth')
