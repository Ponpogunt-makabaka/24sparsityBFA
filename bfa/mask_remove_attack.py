#!/usr/bin/env python3
"""
Task 6: One-Mask Removal Attack

Only flips sparse masks from 1 -> 0 (remove active weights),
keeping weight values unchanged. Uses signed first-order loss increase:
score = grad * delta, where delta = -w (for mask 1 -> 0).
"""
import torch
...
@dataclass
class MaskRemoveAttackResult:
    """Result of one-mask removal attack."""
...
class OneMaskRemovalAttack:
    """
    Attack that only flips sparse mask bits from 1 -> 0.
    This simulates dynamic pruning of active weights.
    """
    ...
    def flip_one_mask_bit(self, layer_name: str, module: nn.Module, mask_idx: int) -> bool:
        """Flip a sparse mask bit only if it is 1 (remove)."""
        with torch.no_grad():
            mask_flat = module.sparse_mask.flatten()
            current_val = int(mask_flat[mask_idx].item())
            if current_val != 1:
                return False
            # Integer/boolean flip: 1 -> 0
    ...
    def compute_one_mask_sensitivity(
        self,
        ...
        """
        Compute sensitivity for removing active mask positions.

        score = grad * delta, where delta = -w (for mask 1 -> 0).
        Only considers mask positions with current mask==1.
        """
    ...
    def progressive_one_mask_search(
        self,
    ...
    ) -> MaskRemoveAttackResult:
        """Run progressive search that only removes one mask bits."""
        ...
        print(f"[MaskRemove] Initial accuracy: {initial_acc:.2f}%, loss: {initial_loss:.4f}")
        print(f"[MaskRemove] {'Flips':>8} | {'Accuracy':>10} | {'Loss':>10} | {'Removed Mask (layer:idx)'}")
        print(f"[MaskRemove] {'-'*8}-+-{'-'*11}-+-{'-'*11}-+-{'-'*60}")
        ...
        with tqdm(total=max_flips, desc="MaskRemove BFA Progress") as pbar:
            while total_flips < max_flips:
                sensitivities = self.compute_one_mask_sensitivity(
                ...
                if not sensitivities:
                    ...
                    if consecutive_no_bits >= max_consecutive:
                        print("[MaskRemove] No more sensitive one-mask bits found")
                        break
                    continue
                ...
                if self.flip_one_mask_bit(layer_name, module, mask_idx):
                ...
                if total_flips % log_interval == 0:
                    ...
                    print(f"[MaskRemove] {total_flips:8d} | {current_acc:9.2f}% | {current_loss:10.4f} | {bit_summary}")
                ...
                if current_acc < target_accuracy * 100:
                    print(f"[MaskRemove] Target accuracy reached: {current_acc:.2f}%")
                    break

        final_acc, final_loss = self._evaluate(test_loader, criterion)
        print(f"[MaskRemove] {'-'*8}-+-{'-'*11}-+-{'-'*11}-+-{'-'*60}")
        print(f"[MaskRemove] Final accuracy: {final_acc:.2f}%, loss: {final_loss:.4f} after {total_flips} flips")
        print(f"[MaskRemove] Total unique removed masks: {len(self.flipped_bits)}")

        return MaskRemoveAttackResult(
...
def run_mask_remove_attack(
    ...
    """Convenience wrapper to run one-mask removal attack."""
    import pickle

    criterion = nn.CrossEntropyLoss()
    attack = OneMaskRemovalAttack(model)

    result = attack.progressive_one_mask_search(
    ...
    if save_path:
        ...
        print(f"[MaskRemove] Results saved to {save_path}")

    if save_log_path:
        with open(save_log_path, 'w', encoding='utf-8') as f:
            f.write("=" * 100 + "\n")
            f.write("Task 6: One-Mask Removal Attack Log\n")
            f.write("=" * 100 + "\n\n")
            ...
            f.write(f"Total Unique Removed Masks: {len(result.mask_positions)}\n\n")
            f.write("-" * 100 + "\n")
            f.write(f"{'Flips':>8} | {'Accuracy':>10} | {'Loss':>10} | {'Removed Mask (layer:idx)'}\n")
            f.write("-" * 100 + "\n")
            ...
            f.write("-" * 100 + "\n\n")
            f.write("All Removed Masks:\n")
            f.write("-" * 100 + "\n")
            for i, (layer, idx) in enumerate(result.mask_positions, 1):
                f.write(f"  {i:3d}. {layer.split('.')[-1]}:{idx}\n")

        print(f"[MaskRemove] Detailed log saved to {save_log_path}")

    return result


if __name__ == '__main__':
    print("One-Mask Removal Attack Engine")
