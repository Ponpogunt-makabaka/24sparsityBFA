#!/usr/bin/env python3
"""
Task 6: Sparse INT8 ResNet-18 (ImageNet) - CSR Index Non-Collision Attack
"""
import argparse
import os
import pickle
...
def main():
    parser = argparse.ArgumentParser(description="Task 6: ResNet-18 CSR Non-Collision Attack")
    ...
    parser.add_argument("--weights-path", type=str, default=None,
                        help="Local ResNet-18 weights path (offline load)")
    ...
    # Apply 2:4 sparsity (skip conv1/fc/downsample, in_ch%4!=0)
    ...
    # Save result
    result_path = "./results/task6_resnet18_csr_non_collision_result.pkl"
    log_path = "./results/task6_resnet18_csr_non_collision_log.txt"
    plot_path = "./results/task6_resnet18_csr_non_collision.png"
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        f.write("=" * 100 + "\n")
        f.write("Task 6: ResNet-18 CSR Non-Collision Attack\n")
        f.write("=" * 100 + "\n\n")
    ...
    plt.plot(flips, result.accuracy_history, marker='o', linewidth=2, markersize=5,
             label='ResNet-18 CSR Non-Collision')
    plt.grid(True, alpha=0.3, linestyle='--')
    plt.xlabel('Successful Flips', fontsize=12, fontweight='bold')
    plt.ylabel('Top-1 Accuracy (%)', fontsize=12, fontweight='bold')
    plt.title('Task 6: ResNet-18 CSR Index Attack (Non-Collision)', fontsize=14, fontweight='bold')
    plt.ylim(0, 100)
    plt.xlim(left=0)
    plt.legend(loc='best')
    plt.tight_layout()
    ...
    print(f"[Task6] Results saved to {result_path}")
    print(f"[Task6] Detailed log saved to {log_path}")
    print(f"[Task6] Plot saved to {plot_path}")
