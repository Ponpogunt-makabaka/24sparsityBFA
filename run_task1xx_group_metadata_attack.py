#!/usr/bin/env python3
"""
Task 1.xx: Group-based 2:4 Metadata Attack (Position Encoding)

This is a NEW attack (1.xx series) that implements a group-local greedy search:
...
Outputs:
- results/new/task1xx_group_metadata_attack_curve.png
- results/new/task1xx_group_metadata_attack_table.csv
- results/new/task1xx_group_metadata_attack_log.txt
- results/new/task1xx_group_metadata_attack_topk_verify.csv (if topk_verify > 0)
"""
...
def run_group_metadata_attack(
    ...
    for step in range(1, max_flips + 1):
        ...
        print(f"[Step {step:3d}] {chosen_candidate.layer_name:30s} g={chosen_candidate.group_idx:6d} "
              ...
              + f" | acc={acc:6.2f}% loss={loss:8.4f} hash={new_hash} t={step_time:.2f}s")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Task 1.xx: Group-based 2:4 Metadata Attack (Position Encoding)"
    )
    ...
    parser.add_argument("--out-prefix", type=str, default="results/new/task1xx_group_metadata_attack")

    args = parser.parse_args()

    # Set up output directory
    os.makedirs(os.path.dirname(args.out_prefix) or "results/new", exist_ok=True)
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "baseline_acc", "final_acc", "max_flips", "encoding",
    ...
    with open(log_path, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("Task 1.xx: Group-based 2:4 Metadata Attack (Position Encoding)\n")
        f.write("=" * 80 + "\n")
        f.write(f"Script: run_task1xx_group_metadata_attack.py\n")
        ...
        for log in result['step_logs']:
            ...
            f.write(f" | acc={log.accuracy:.2f}% loss={log.loss:.4f} hash={log.metadata_hash}\n")
        f.write("\n" + "=" * 80 + "\n")
        f.write("Reproduction command:\n")
        f.write(f"  python run_task1xx_group_metadata_attack.py \\\n")
...
# DEPRECATED: This script has been renamed to run_R1_T01_group_metadata_index_anypattern.py
# Please use the new R1_T01 script following the revised naming convention.
