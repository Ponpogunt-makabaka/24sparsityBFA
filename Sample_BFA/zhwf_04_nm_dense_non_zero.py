...
if __name__ == "__main__":
	...
	parser.add_argument('--topk', type=int, default=20, help='number of top gradient indices to consider per layer')
	...
	for it in range(n_iter):
		...
		# sort candidates by delta descending and pick the first whose original INT8 is non-zero
		...
		for rank, entry in enumerate(sorted_entries):
			...
			if rank == 0 and orig_int8_tmp == 0:
				print('Top candidate had INT8 == 0; non-zero flip not the best in this iter.')
				top_was_zero = True
			if orig_int8_tmp != 0:
				chosen = entry
				break
		...
		if chosen is None:
			print('No non-zero INT8 candidate found this iteration; stopping.')
			break

		global_max = chosen
		# print global best non-zero clip and int8 value before/after candidate flip
		...
		print(f"Global best non-zero clip: module={global_max['module_name']} target={global_max['target_name']} index={global_max['multi_idx']} delta={global_max['delta']:+.6f}")
