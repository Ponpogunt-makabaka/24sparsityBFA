...
model_names = sorted(name for name in pth.__dict__
                     if name.islower() and not name.startswith("__")
                     and callable(pth.__dict__[name]))
...
def main():
    ...
    net = pth.__dict__[args.arch](num_classes)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            # allow unpickling RecorderMeter and other local classes when loading full checkpoint
            try:
                torch.serialization.add_safe_globals([RecorderMeter])
            except Exception:
                pass
            # Load checkpoint; handle CUDA/device mismatch by falling back to CPU
            try:
                if args.use_cuda and torch.cuda.is_available():
                    checkpoint = torch.load(args.resume, weights_only=False)
                else:
                    checkpoint = torch.load(args.resume, map_location=torch.device('cpu'), weights_only=False)
            except RuntimeError as e:
                msg = str(e)
                if 'Attempting to deserialize object on CUDA device' in msg or 'torch.cuda.device_count() is 0' in msg:
                    print_log("CUDA device mismatch when loading checkpoint; retrying on CPU", log)
                    checkpoint = torch.load(args.resume, map_location=torch.device('cpu'), weights_only=False)
                else:
                    raise
            ...
            # checkpoint may store the model state under different keys
            if 'model_state_dict' in checkpoint:
                state_dict_src = checkpoint['model_state_dict']
            elif 'state_dict' in checkpoint:
                state_dict_src = checkpoint['state_dict']
            else:
                # if the checkpoint appears to be a raw state_dict (tensor->param mapping), use it
                # otherwise avoid updating with the entire checkpoint which may contain metadata
                # like 'epoch', 'acc', etc.
                # Heuristic: check if all values look like tensors
                is_raw_state = all(hasattr(v, 'shape') for v in checkpoint.values())
                if is_raw_state:
                    state_dict_src = checkpoint
                else:
                    # fallback: try common nested key names
                    state_dict_src = checkpoint.get('model', checkpoint.get('net', {}))

            if not state_dict_src:
                raise RuntimeError(f"No model state found in checkpoint '{args.resume}'")

            state_tmp.update(state_dict_src)

            net.load_state_dict(state_tmp)

            # detect whether the loaded checkpoint already contains quantization params
            # (we'll use this to avoid recomputing step_size/resetting weights)
            try:
                ck_state = state_dict_src
            except Exception:
                ck_state = {}
            ck_is_quantized = any(('step_size' in k or 'b_w' in k) for k in ck_state.keys())
            # attach flag for later use
            checkpoint['__is_quantized__'] = ck_is_quantized
    ...
    # Only recompute step sizes if the checkpoint wasn't already a quantized checkpoint.
    # Recomputing step sizes for an already-quantized (integer-coded) checkpoint will
    # change the scaling and break accuracy.
    ck_is_quantized = checkpoint.get('__is_quantized__', False) if isinstance(checkpoint, dict) else False
    if not ck_is_quantized:
    ...
    # Only run weight reset when explicitly requested and the checkpoint is not already quantized.
    if args.reset_weight and not ck_is_quantized:
        for m in net.modules():
            if isinstance(m, quan_Conv2d) or isinstance(m, quan_Linear):
                m.__reset_weight__()
                # print(m.weight)
...
def perform_attack(attacker, model, model_clean, train_loader, test_loader,
    ...
    for i_iter in range(N_iter):
        ...
        # print(attack_log)
        # append attack_log rows to df (pandas.DataFrame.append removed in newer pandas)
    ...
    column_list = ['module idx', 'bit-flip idx', 'module name', 'weight idx',
                  'weight before (float)', 'weight after (float)', 'INT before', 'INT after',
                  'int diff', 'hamming distance', 'quant step', 'validation accuracy',
                  'accuracy drop']
...
def validate(val_loader, model, criterion, log, summary_output=False):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            if summary_output:
                ...
                output_summary.extend(tmp_list.tolist())
    ...
    if summary_output:
        output_summary = np.asarray(output_summary)
