...
class BFA(object):
    def __init__(self, criterion, model, k_top=10):
        ...
        # helper to detect quantized conv/linear modules when quan_* types are not class objects
        def _is_quant_module(mod):
            return hasattr(mod, 'weight') and (hasattr(mod, 'N_bits') or hasattr(mod, 'b_w') or hasattr(mod, 'step_size'))
        self._is_quant_module = _is_quant_module

        # attributes for random attack
        self.module_list = []
        for name, m in model.named_modules():
            if _is_quant_module(m):
    ...
    def progressive_bit_search(self, model, data, target):
        ...
        while self.loss_max <= self.loss.item():
            ...
            for name, module in model.named_modules():
                if self._is_quant_module(module):
            ...
            if not self.loss_dict:
                # nothing recorded -> cannot proceed further
                break
        ...
        if 'max_loss_module' not in locals():
            return []
        for module_idx, (name, module) in enumerate(model.named_modules()):
            if name == max_loss_module:
                ...
                for i in range(attack_weight_idx.size()[0]):
                    ...
                    print('weight before PBS attack:', weight_prior)
                    print('weight after PBS attack:', weight_post)
                    ...
                    tmp_list = [module_idx,
                                ...
                                weight_prior,
                                weight_post,
                                int(weight_prior / quant_step) if quant_step else 0,
                                int(weight_post / quant_step) if quant_step else 0,
                                int(weight_post / quant_step - weight_prior / quant_step) if quant_step else 0,
                                1,
                                quant_step]
